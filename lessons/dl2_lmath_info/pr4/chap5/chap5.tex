...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Loi / Paramètres} & \textbf{Espérance} & \textbf{Variance} \\
\hline
Bernoulli $\mathcal{Ber}(p)$ & $p$ & $p(1-p)$ \\
\hline
Uniforme $\mathcal{U}(\{1, \ldots, n\})$ & $\dfrac{n+1}{2}$ & $\dfrac{n^2-1}{12}$ \\
\hline
...
\hline
Hypergéométrique $\mathcal{H}(N,n,K)$ & $\dfrac{nK}{N}$ & $n\dfrac{K}{N}\left(1-\dfrac{K}{N}\right)\dfrac{N-n}{N-1}$ \\
\hline
\end{tabular}
