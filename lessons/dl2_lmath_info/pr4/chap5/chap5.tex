...
\begin{itemize}
    ...
    \item Si on partitionne $\mathbb{N}^*$ en une collection dénombrable $(I_p)_{p\in\mathbb{N}}$ d'ensembles infinis, alors $\sum_{n \geq 1} a_n = \sum_{p \geq 1} \sum_{n \in I_p} a_n$
\end{itemize}
}
...
\definition{
    Soit $X$ une variable aléatoire définie sur un espace probabilisé ($\Omega$, $\mathbb{P}$) et à valeurs dans $\mathbb{R}_+$.
...
\remark{En vocabulaire sur les séries, on dit que $X$ admet une espérance si la série $\sum_{x \in E} x \cdot \mu_X(\{x\})$ est absolument convergente.}
...
\hline
Géométrique $\mathcal{G}(p)$ sur $\mathbb{N*}$ & $p(1-p)^{k-1}$ & $\dfrac{1}{p}$ & $\dfrac{1-p}{p^2}$ \\
\hline
...
\theorem{Proposition}{Espérance du produit de variables aléatoires indépendantes}{false}{
    Soient $X$ et $Y$ deux variables aléatoires indépendantes à valeurs dans $E$ et $F$ deux ensembles dénombrables respectivement.
    \begin{enumerate}
