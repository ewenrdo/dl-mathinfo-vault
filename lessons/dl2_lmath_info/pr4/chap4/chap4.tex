...
\definition{
    Soient $(X_1, \ldots, X_n)$ $n$ variables aléatoires définies sur $\mathbb{K}$ et à valeur dans $(E_1, \ldots, E_n)$.\\
    On dit que ces variables aléatoires sont indépendantes si pour tout ensembles $F_1 \subset E_1, \ldots, F_n \subset E_n$ on a :\\
    \[
        \mathbb{P}({x_1 \in F_1} \cap {x_2 \in F_2} \cap \ldots \cap {x_n \in F_n}) = \prod_{i=1}^n \mathbb{P}(X_i=x_i)
    \]
}

\theorem{Proposition}{Indépendance de deux variables}{false}{
    Soient $X, Y$ deux variables aléatoires.
    Si $\forall A,B \; \mathbb{P}(X^{-1}(A) \cap Y^{-1}(B)) = \mathbb{P}(X^{-1}(A)) \cdot \mathbb{P}(Y^{-1}(B))$, alors $X$ et $Y$ sont indépendantes.
}
...
\definition{
    La \textbf{loi de Bernouilli} de paramètre $p \in [0, 1]$ est la mesure de probabilité sur $\{0, 1\}$ définie par $\mu(\{0\}) = 1-p$ et $\mu(\{1\}) = p$.\\
    On la note $Ber(p)$ ou $\mathcal{B}(p)$.
}
...
\attention{L'ensemble d'étude de la loi géométrique fait débat. Si on la définit sur $\mathbb{N}^{*}$, alors $\mu(\{n\}) = p \cdot (1-p)^{n-1}$ pour tout $n \in \mathbb{N}^{*}$. Il s'agit de faire une transformation affine de la variable aléatoire $N$ qui suit la loi géométrique sur $\mathbb{N}$ : $N' = N - 1$.}
