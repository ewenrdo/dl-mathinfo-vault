...
\definition{
    On note $\mu_n$ la loi de $X_n$, \textit{i.e.} $\mu_n(k) = \mathbb{P}(X_n = k)$ et $\mu_n = (\mathbb{P}(X_n = k))_{k \in E}$.\\
    $\mu_n$ est un vecteur ligne de probabilité.
}
\example{
    Supposons $X_0 \sim Ber(p)$, alors $\mu_0 = (1-p, p)$.
}
...
\noindent{\textbf{Démonstration : }\\
    $\mathbb{P}(X_{n+1}=j)=\sum_{k\in E}\mathbb{P}(X_{n+1}=j|X_n=k)\mathbb{P}(X_n=k)=\sum_{k\in E}p_{kj} \mathbb{P}(X_n=k)=\sum_{k\in E}\mu_n(k)p_{kj}$\\
    $=(\mu_n(1), \mu_n (2), ..., \mu_n (l))\cdot    \begin{pmatrix}
        p_{11} && \cdots && p_{1f} \\ \vdots && \ddots && \vdots \\ p_{\ell1} && \cdots && p_{\ell f}
    \end{pmatrix}   =(\mu_{n+1}(1), ...,\mu_{n+1}(l))$.\\
}

\definition{
    La \textbf{matrice de transition à $n$ pas} est définie par $P^{(n)}_{ij} = \mathbb{P}(X_n = j | X_0 = i)$.
}
\theorem{Proposition}{Relation entre $P^{(n)}$ et $P$}{false}{
    Soit $(X_n)_{n \in \mathbb{N}}$ une chaîne de Markov \textbf{homogène} sur un ensemble d'états $E$. Alors :
    \[ P^{(n)} = P^n \]
...
\noindent{\textbf{Démonstration :} Par récurrence
    \begin{itemize}
        \item $n=0$ : $P^{(0)}=P^0=Id$.
        \item On utiliser le fait que $P^{(n)} = P^n \Leftrightarrow P^{(n)}_{ij} = P_{ij}^n$ pour tout $i,j \in E$.\\
        On suppose que $P{(n)}=P^n$.\\
        $P_{ij}^{(n+1)}=\mathbb{P}(X_{n+1}=j|X_0=i)
            =\sum_{k\in E}\mathbb{P}(X_{n+1}=j, X_n=k|X_0=i)$\\
            $=\sum_{k\in E} \frac{\mathbb{P}(X_{n+1}=j,X_n=k, X_0=i)}{\mathbb{P}(X_0=i)} 
...
\theorem{Théorème}{Existence de lois stationnaires}{true}{
...
\theorem{Proposition}{Relation de communication}{false}{
...
\subsection{Graphes de transition}

\example{
    Soit $E = \{1,2,3\}$ et $P = \begin{pmatrix} 0 & 1 & 0 \\ \frac{1}{2} & 0 & \frac{1}{2} \\ 0 & 0 & 1 \end{pmatrix}$.\\
    ...
    \begin{center}
        \begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
           ...
           \path[->] (1) edge[bend left] node[midway, above] {1} (2);
           \path[->] (2) edge[bend left] node[midway, below] {0.5} (1);
           \path[->] (2) edge node {0.5} (3);
           \path[->] (3) edge [loop below] node {1} ();
        \end{tikzpicture}
    \end{center}
}

\remark{Dans le graphe, on peut alors lire, entre autres :
    \begin{itemize}
        \item $1$ communique vers $2$ (car il y a une flèche de $1$ vers $2$),
        \item la chaîne de Markov n'est pas irréductible (car $3$ est un état absorbant qui ne communique pas avec les autres),
        \item les périodes sont $d(1) = 2$, $d(2) = 2$ et $d(3) = 1$.
    \end{itemize}
}

\subsection{Ergodicité}

\theorem{Théorème d'ergodicité}{}{true}{
