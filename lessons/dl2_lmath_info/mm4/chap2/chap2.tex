...
D'où : $\langle f_j, e_i \rangle = \left\langle \sum_{k=1}^n p_{kj} \cdot \varepsilon_k^*, \sum_{l=1}^n c_{li} \cdot \varepsilon_l \right\rangle = \sum_{k=1}^n \sum_{l=1}^n p_{kj} c_{li} \langle \varepsilon_k^*, \varepsilon_l \rangle = \sum_{k=1}^n p_{kj} c_{ki}$.\\
\end{document}