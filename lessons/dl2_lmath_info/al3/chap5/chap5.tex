...
\definition{
    Soit $E$ un $K$-espace vectoriel de dimension finie $n$, et soit $B = (e_1,\ldots,e_n)$ une base de $E$.\\
    Soit $(u_1,\ldots,u_n) \in E^n$.\\
    On appelle \textbf{déterminant} de la famille $(u_1,\ldots,u_n)$ dans la base $B$ eton note $det_B(u_1,\ldots,u_n)$ la valeur de la forme $n$-linéaire alternée $f_0$ (définie précédemment) en $(u_1,\ldots,u_n)$.\\\\

    Autrement dit, si $det_B(u_1,\ldots,u_n) = f_0(u_1,\ldots,u_n)$ où $f_0$ est la forme $n$-linéaire alternée telle que $f_0(e_1,\ldots,e_n) = 1$.\\
    si on pose $u_j = \sum_{i=1}^n \lambda_{ij} e_i$ avec $\lambda_{ij} \in K$, on a :\\
}

\theorem{Proposition}{}{true}{
    L'application $(u_1,\ldots,u_n) \mapsto det_B(u_1,\ldots,u_n)$ est une forme $n$-linéaire alternée sur $E$.\\
    En particulier, elle change de signe si on échange deux de ses arguments.\\
}
\theorem{Proposition}{Combinaison linéaire et déterminant}{true}{
    On a que $(u_1, \ldots, u_n)$ est liée si $det_B(u_1, \ldots, u_n) = 0$.
    (Et donc $(u_1, \ldots, u_n)$ est une base de $E$ si $det_B(u_1, \ldots, u_n) \neq 0$).
}

\theorem{Théorème}{Déterminants dans deux bases}{true}{
    Soient $B_1$ et $B_2$ deux bases de $E$.\\
    On a $det_{B_2}(u_1,\ldots,u_n) = det_{B_2}(B_1) \times det_{B_1}(u_1,\ldots,u_n)$.
}
\section{Déterminant d'une matrice 3x3}
\ndlr{Voir MM1, méthode de Sarrus}

\section{Déterminant d'une matrice carrée}

\definition{
    Le déterminant d'une matrice $A$ est le déterminant dans la base canonique de la famille de vecteurs constituée par les colonnes de $A$.
}
\example{
    Soit $A = \begin{pmatrix} a & b \\ c & d \end{pmatrix} \in M_2(K)$.\\
    On a $det(A) = det_{B_{can}} \left( \begin{pmatrix} a \\ c \end{pmatrix}, \begin{pmatrix} b \\ d \end{pmatrix} \right) = ad - bc$.
}

\theorem{Proposition}{Matrices triangulaires}{false}{
    Si $A = (a_{ij}) \in M_n(K)$ est triangulaire (en particulier diagonale), on a $det(A) = a_{11} \times a_{22} \times \ldots \times a_{nn}$.\\\\
    Autrement dit, le déterminant d'une matrice triangulaire est le produit des scalaires sur sa diagonale.
}

\theorem{Théorème}{Transposée}{true}{
    Pour toute matrice carrée $A \in M_n(K)$, on a $det({ }^t A) = det(A)$.
}
...
\theorem{Proposition}{}{false}{
    ...
    On a donc $A_{n,n} = \begin{pmatrix}
     & 0 \\
    (*) & \vdots \\
     & 0 \\
     & a_{nn}
...
\noindent Posons $A_{ij} \in M_{n-1}(K)$ la matrice obtenue en supprimant la $i$-ème ligne et la $j$-ème colonne de $A$.\\
\vocabulary{
    \begin{enumerate}
...
\theorem{Théorème}{}{false}{
    On a $det(A) = (-1)^{i+1}a_{i1} \Delta_{i1} + \ldots + (-1)^{i+n}a_{in}\Delta_{in}$ (développement suivant la $i$-ème ligne).\\
    $= (-1)^{1+j}a_{1j} \Delta_{1j} + \ldots + (-1)^{n+j}a_{nj}\Delta_{nj}$ (développement suivant la $j$-ème colonne).\\\\

    Autrement dit, $det(A) = \sum_{j=1}^n (-1)^{i+j} a_{ij} \Delta_{ij} = \sum_{i=1}^n (-1)^{i+j} a_{ij} \Delta_{ij}$.
}
...
On se donne $A = (a_{ij}) \in M_n(K)$ et $b \in K^n$. Soit $B = \begin{pmatrix}b1\\ \vdots \\ b_n\end{pmatrix}$.\\
On cherche les solutions $X \in K^n$ à l'équation $AX = B$.
En posant $X = \begin{pmatrix}x_1 \\ \vdots \\ x_n\end{pmatrix}$, on a le système linéaire suivant :
\[\begin{cases}
...
\theorem{Théorème}{}{false}{
    Si $\det(A) \neq 0_K$, alors le système admet une unique solution donnée par $X = A^{-1}B$ (formule de Cramer).\\
    Plus précisément, la solution $X = \begin{pmatrix}x_1 \\ \vdots \\ x_n\end{pmatrix}$ vérifie pour tout $k$ :\\
    $x_k = \frac{det(A_k(B))}{det(A)}$ où $A_k(B)$ est la matrice obtenue en remplaçant la $k$-ème colonne de $A$ par le vecteur $B$.\\
    Si $\det(A) = 0_K$, le système admet soit aucune, soit une infinité de solutions.
}

\example{Pour $n=2$, $A = \begin{pmatrix} a & b \\ c & d \end{pmatrix}$ et $B = \begin{pmatrix} b_1 \\ b_2 \end{pmatrix}$.\\
On a : $det(A) = ad - bc$.\\
Si $ad - bc \neq 0$, on a $\begin{cases} x_1 = \frac{b_1 d - b_2 b}{ad - bc} \\ x_2 = \frac{a b_2 - c b_1}{ad - bc} \end{cases}$.
}
\end{document}