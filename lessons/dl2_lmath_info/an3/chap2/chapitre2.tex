...
\definition{
    ...
    Pour tout \(N \in \mathbb{N}\) on pose \(S_N = \sum_{n=0}^{N} u_n \in \mathbb{K}\).\\\\
    La suite \((S_N)_{N\in\mathbb{N}}\) est appelée la \textbf{série} de terme général \(u_n\) et se note \(\sum_{n\geq0} u_n\).\\
    Les \(S_N\) sont appelées les \textbf{sommes partielles} de la série.
}
...
\definition{
    Soit $(u_n)$ défini à partir d'un rang $n_0\in\mathbb{N}$. Une troncature de la série $\sum_{n\geq0}u_n$ au rang $n_0$ est la série \(\sum_{n\geq n_0} u_n\).\\
    Les sommes partielles associées sont \(S_N=\sum_{n=n_0}^{N} u_n\).
}

%\definition{
%    Si $(u_n)$ est une suite définie pour \(n \geq n_0 \in \mathbb{N}\), alors on peut considérer la série \(\sum_{n \geq n_0} u_n\) où \(u_{0} = u_{1} = ... = u_{n_0 - 1} = 0\), ou bien on peut écrire $\sum_{n \geq n_0} u_n$.
...
\example{Calculons la valeur de $\sum_{n\geq0}^{+\infty} \frac{1}{n!}$.\\
