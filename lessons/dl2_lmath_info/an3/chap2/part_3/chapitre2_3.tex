...
\attention{À changer dans la démo $A_N$ en $A_N'$ et $B_N$ en $B_N'$.\\
...
La série de Talyor associée à $f$ est donc $\sum_{n\geq0} \frac{f^{(n)}(0)}{n!} x^n = 0 \forall x \in \mathbb{R}$.\\
Et donc on a pas $f(x) = \sum_{n\geq0}^{+\infty} \frac{f^{(n)}(0)}{n!} x^n$ sauf en 0.\\
}
\end{comment}
\end{document}
