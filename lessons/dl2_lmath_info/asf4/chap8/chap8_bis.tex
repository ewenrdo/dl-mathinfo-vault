...
\remark{Le déterminant représente l'aire du parallélogramme formé par les vecteurs $x$ et $y$, avec un signe qui dépend de l'orientation (sens trigonométrique ou sens horaire).}
...
\theorem{Formule de Rodrigues}{}{false}{
    Soit $u \in SO_3(\mathbb{R})$ une rotation d'angle $\theta$ autour de l'axe dirigé par le vecteur unitaire $d$. Alors pour tout $x \in \mathbb{R}^3$, on a :
    \[
        u(x) = \cos \theta x + (1 - \cos \theta) \langle x, d \rangle d + \sin \theta (d \wedge x)
    \]
}
\noindent{\textbf{Preuve :}\\
Décomposons $x$ en $x = x_1 + x_2$, où $x_1 \in D$ et $x_2 \in D^\perp$.\\
Alors $u(x) = u(x_1) + u(x_2)$.\\
Or $u(x_1) = x_1$ et $u(x_2) = \cos \theta x_2 + \sin \theta (d \wedge x_2)$, d'où $u(x) = \cos \theta x + (1 - \cos \theta) \langle x, d \rangle d + \sin \theta (d \wedge x)$. $\Box$
}

\remark{La formule de Rodrigues est très utilisée en physique pour calculer la rotation d'un vecteur dans l'espace.}
\ndlr{Cette formule est dans le polycopié de Romain Petrides, mais n'a pas été abordée en cours. Mais le nom était cool, alors je l'ai mise.}
