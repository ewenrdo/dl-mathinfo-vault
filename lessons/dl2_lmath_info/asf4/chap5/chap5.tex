...
%∑_(n≥1)▒〖na_n z^(n-1) 〗 a le même rayon de convergence que ∑_(n=0)^(+∞)▒〖a_n z^n 〗. 					
%(≈∑▒〖P(n)/Q(n)  a_n z^n 〗 et ensuite proposition sur les fractions rationnelles de polynômes)
%Or, si on pose f_n (z)=a_n z^n, alors f_n^' (z)=na_n z^n.
%Comme ∑▒〖f_n (z) 〗 converge normalement sur D(0,r) pour r<R, 			∑▒〖f_n^' (z) 〗 converge normalement sur D(0,r) pour r<R.
%Le théorème de dérivations des séries complexe impose que f est C-dérivable et f^' (z)=∑_(n=1)^(+∞)▒〖na_n z^(n-1) 〗.
%Montrons que (z^n )^'=nz^(n-1).
%On a z^n-z_0^n=(z-z_0 ) ∑_(k=0)^(n-1)▒〖z^k z_0^(n-1-k) 〗, 
%Donc (z^n-z_0^n)/(z-z_0 )=∑_(k=0)^(n-1)▒〖z^k z_0^(n-k-1) 〗 □(→┬(z→z_0 ) ) ∑_(k=0)^(n-1)▒z_0^(n-1) =nz_0^(n-1).
