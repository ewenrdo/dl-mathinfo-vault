...
\begin{enumerate}
    ...
    \textbf{Démonstration} : Soit $\varepsilon>0$.\\
    Pour $ \widetilde{(f_n)}(1)=1$, il existe $ x\in [0, 1[$ tel que $ f_n(x)\ge1-\varepsilon$.\\
    Donc, $\|f_n\|_{[0, 1], \infty}\ge f_n(x)\ge 1-\varepsilon$. (la borne supérieur est un majorant).\\
    Donc, $\varepsilon \to 0\Rightarrow\|f_n\|_{[0, 1], \infty}\ge 1$. $\Box$
\end{enumerate}
}
...
\noindent{\textbf{Démonstration : }\\
    $|\int_a^b (f_n(x)-f(x)) dx|\le\int_a^b |f_n(x)-f(x)| dx\le \|f_n-f\|_{[a, b], \infty} \int_a^b dx$\\
    $|\int_a^b f_n(x) dx - \int_a^b f(x) dx|\le(b-a)\|f_n-f\|_{[a, b], \infty}\to_{n\to +\infty} 0$. $\Box$
}
...
\noindent{\textbf{Démonstration :}\\
    \begin{itemize}
        \item $|f(x)|\le \|f_n(x)\|_{I, \infty}$ et $\|f_n\|_{I, \infty}$ converge. Donc $()\sum_n|f_n(x)|$ converge.\\ 
        Donc, $ \sum_nf_n(x)$ converge absolument.
        \item Montrons que $(\sum_{k=0}^n|f(x)|)_{n\in\mathbb{N}}$ est uniformément de Cauchy. Soit $p>q$ et $x\in I$.\\
        $|\sum_{k=0}^qf_n(k)-\sum_{k=0}^pf_n(k)|=|\sum_{k=p+1}^qf_n(k)|$\\
        $\le\sum_{k=p+1}^q|f_n(k)|\le\sum_{k=p+1}^q\|f_k\|_{I, \infty}\le\sum_{k=p+1}^{+\infty}\|f_k\|_{I, \infty}$\\
        Comme $\sum_{n=0}^{+\infty}\|f_n\|_{I, \infty}<+\infty$, on a $\lim_{n\to+\infty}\sum_{k=n+1}^{+\infty}\|f_n\|_{I, \infty}=0$\\
        En passant au sup, $\|\sum_{k=0}^qf_k-\sum_{k=0}^pf_k\|_{I, \infty}\le\sum_{k=p+1}^{+\infty}\|f_n\|_{I, \infty}\to_{p\to+\infty}0$\\
        Donc, $\sum_{k=0}^nf_k$ est uniformément de Cauchy. Donc converge uniformément.
        \item $|\sum_{k=0}^nf_k(x)|\le\sum_{k=0}^n|f_k(x)|\le\sum_{k=0}^n\|f_k\|_{I, \infty}\le\sum_{k=0}^{+\infty}\|f_k\|_{I, \infty}$.\\
        En faisant $n\to+\infty$, par convergence simple, $|\sum_{k=0}^{+\infty}f_k(x)|\le\sum_{k=0}^{+\infty}\|f_k\|_{I, \infty}$.\\
        Donc, $\sum_{k=0}^{+\infty}f_k$ est bornée par $\sum_{k=0}^{+\infty}\|f_k\|_{I, \infty}$. $\Box$
    \end{itemize}
}
...
\noindent{\textbf{Démonstration :} On note $V_n=\sum_{k=0}^nv_k$ et $\|v_k\|_\infty<M$.\\
Montrons que $\sum u_nv_n$ est uniformémentde Cauchy. Soit $x\in I$.\\
$\sum_{n=q+1}^pu_nv_n(x)\le\sum_{n=q=1}^(p-1)|u_{n+1}-u_n| |V_n(x)|+|u_p||V_q(x)|\le\|V_n\|_\infty[\sum_{n=q+1}^{p-1}|u_{n+1}-u_n|+|u_p|+|u_q|]$\\
Comme $(u_n)$ est décroissante, $|u_{n+1}-u_n|=u_n-u_{n+1}$.\\ 
D'où, $|\sum_{n=q+1}^pu_nv_n(x)\le M[\sum_{n=q+1}^{p-1}(u_n-u_{n+1})+u_p+u_q]=M[u_{q+1}-u_p+u_p+u_q]=M(u_{q+1}+u_q)$.\\
Soit $n_0\in\mathbb{N}, \forall n\ge n_0, |u_n|\le\varepsilon/2M$. Alors pour $p, q\ge n_0$, $\|\sum_{n=q+1}^pu_nv_n\|_\infty\le M\times(\varepsilon/2M+\varepsilon+2M)\le\varepsilon$.\\
Donc, $\sum u_nv_n$ est uniformément de Cauchy. Donc $\sum u_nv_n$ converge uniformément.
}
...
\example{
    ...
    $\sum_{k=0}^n sin(kt) = Im(\sum_{k=0}^n e^{ikt}) = Im(\frac{1 - e^{i(n+1)t}}{1 - e^{it}}) 
    = Im(\frac{e^{i(n+1)t/2}(e^{-i(n+1)t/2}-e^{i(n+1)t/2})}{e^{it/2}(e^{-it/2}-e^{it/2})}) = Im(\frac{e^{i(n+1)t/2}}{e^{it/2}}\times\frac{-2i sin(\frac{n+1}{2}t)}{-2i sin(t/2)}) = Im(e^{int/2}\times\frac{sin(\frac{n+1}{2}t)}{sin(t/2)})
    =  \frac{sin(\frac{nt}{2} \cdot sin(\frac{(n+1)t}{2})}{sin(\frac{t}{2})}$\\
