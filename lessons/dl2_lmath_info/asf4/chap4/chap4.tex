...
\noindent{ \textbf{Démonstration :}\\
    \begin{itemize}
        ...
        \item   Réciproquement, si $x$ et $y$ sont colinéaires.\\
                ...
                $\sqrt{\langle x, x\rangle}=\sqrt{\langle\lambda y, \lambda y\rangle}=\sqrt{\lambda^2\langle y, y\rangle}=\lambda\langle y, y\rangle$. D'où l'égalité. 
    \end{itemize}
}
...
\ndlr{cf. Laurent pour les preuves de ces deux dernières propriétés.}
...
\ndlr{cf. Laurent pour ce qui s'ensuit. cf. OneNote pour le schéma.}
%On a ‖x-z‖^2=‖x-p(x)+p(x)-z‖^2=‖x-p(x)‖^2+‖p(x)-z‖^2 par Pythagore				
    %≥‖x-p(x)‖^2. Donc p(x) atteint le min┬(z∈F)⁡‖x-z‖.					
%De plus, le minimum est unique car ‖x-z‖=min┬(z∈F)⁡‖x-z‖
    %⇒‖p(x)-z‖^2=0⇒p(x)=z.
...
\theorem{Théorème}{}{false}{
    ...
    Alors $E = F \oplus F^\perp$ (c'est même une somme directe orthogonale) et $F^\perp = (F^\perp)^\perp$.
}
