...
\definition{
    ...
    \begin{enumerate}
        ...
        \item $\forall x \in E, \; \langle x,x \rangle \geq 0$ (positivité) et $\langle x,x \rangle = 0 \Rightarrow x = 0$ (définie)
    \end{enumerate}
    En résumé, un produit scalaire est une forme bilinéaire, symétrique et définie positive.
}

\remark{Le terme \textbf{"définie"} signifie que $\langle x,x \rangle = 0 \Rightarrow x = 0$. Il s'agit bien du produit scalaire d'un point avec lui-même.}
\attention{De même pour la positivité, on applique le produit scalaire à un point avec lui-même. Ainsi, si $\langle x,y \rangle < 0$ est possible.}
...
\noindent{\textbf{Preuve :} Vérifions avec $\|x\|=\sqrt{\langle x, x\rangle}$.\\
    $\|x+y\|^2=\|x\|^2+2\langle x, y\rangle+\|y\|^2$ et $\|x-y\|^2=\|x\|^2-2\langle x, y\rangle+\|y\|^2$.\\
    Donc, $\|x+y\|^2+\|x-y\|^2=2(\|x\|^2+\|y\|^2)$.
}

\theorem{Proposition}{Identité de polarisation}{false}{
    Soit $E$ un $\mathbb{R}$-espace vectoriel, muni d'un produit scalaire $\langle \cdot, \cdot \rangle$.\\
    Alors $\forall x,y \in E, \; \langle x,y \rangle = \frac{1}{2}(\| x+y \|^2 - \| x \|^2 - \| y \|^2)$.
}
\noindent{\textbf{Preuve :} $\| x+y \|^2 = \langle x+y, x+y \rangle = \langle x,x \rangle + 2\langle x,y \rangle + \langle y,y \rangle = \| x \|^2 + 2\langle x,y \rangle + \| y \|^2$.\\
    Donc $\langle x,y \rangle = \frac{1}{2}(\| x+y \|^2 - \| x \|^2 - \| y \|^2)$. $\Box$
}

\remark{Il existe une autre formule de polarisation : $\langle x,y \rangle = \frac{1}{4}(\| x+y \|^2 - \| x-y \|^2)$ qui sera très pertinente au chapitre 10 sur les formes quadratiques.}
...
\noindent{\textbf{Preuve :} $\| \sum_{i=1}^k x_i \|^2 = \langle \sum_{i=1}^k x_i, \sum_{j=1}^k x_j \rangle = \text{ (bilinéarité) } \sum_{i=1}^k \sum_{j=1}^k \langle x_i, x_j \rangle = \sum_{i=1}^k \langle x_i, x_i \rangle = \sum_{i=1}^k \| x_i \|^2$.\\\\
Pour $k = 2$, 
...
\noindent{\textbf{Preuve :} Soit $\sum_{i\in J}\lambda_ix_i=0$ une combinaison linéaire nulle de vecteur de la famille $J$, avec $J\subset I$ un ensemble fini. \\
...
\remark{Si $(e_i)_i$ est une \textit{b.o.n} de $E$, alors $\forall x \in E, \; x = \sum_i \langle x, e_i \rangle e_i$. On retrouve la structure duale de $E$ à travers le produit scalaire. En particulière, la $i$-ème coordonnée de $x$ dans la base $(e_i)_i$ est $\langle x, e_i \rangle$.}
...
\noindent{\textbf{Preuve :} On procède par récurrence sur $k$.\\
...
\toTrain{Exercice 9 de la feuille de TD}
...
\noindent{\textbf{Preuve :} Soit $(e_1, \ldots, e_n)$ une base orthonormée de $E$.\\
...
\noindent{\textbf{Preuve :} La plus petite distance entre $x$ et un plan est la droite perpendiculaire au plan passant par $x$. $\Box$}
...
\noindent{\textbf{Preuve :} $F^\perp = ker p$ et $F = Im p$ où $p$ est la projection orthogonale de $E$ sur $F$.\\
...
\noindent{\textbf{Preuve :} Par définition d'un hyperplan, et comme $H^\perp$ est un supplémentaire de $H$, il existe $x_0 \in H^\perp$ tel que $H^\perp = Vect(x_0)$.\\
...
\noindent{\textbf{Preuve :} $j_E$ est linéaire et $dimE = dimE^*$ et $x \in ker(j_E) \Leftrightarrow \forall y \in E, \; \langle x,y \rangle = 0 \Leftrightarrow x = 0$. $\Box$}
...
\ndlr{En cours magistral a été abordée la notion d'espace hermitien, mais ce n'est pas au programme.}

%\section{Cas des $\mathbb{C}$-espaces vectoriels : espaces hermitiens}
%
%\ndlr{Cette section ne semble pas être au programme.}
%
%\definition{
