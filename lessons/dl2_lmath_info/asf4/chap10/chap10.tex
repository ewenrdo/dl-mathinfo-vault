...
\docTitle{Chapitre 10 : Formes bilinéaires et quadratiques}
...
\definition{
    ...
    \end{itemize}
    \textit{Autrement dit, $f$ est linéaire par rapport à chacune de ses variables.}
}
...
\section{Formes bilinéaires symétriques}

\subsection{Définitions et propriétés}

\definition{
    Soit $\varphi \in \mathcal{B}(E)$.\\
    On dit que $\varphi$ est \textbf{symétrique} si $\varphi(x,y) = \varphi(y,x)$ pour tous $x,y \in E$.
}
...
\definition{
    Soit $\varphi \in \mathcal{B}(E)$, supposons $\varphi$ symétrique. Alors :
    \begin{enumerate}
        \item $ker(\varphi) = \{ x \in E \mid \forall y \in E \varphi(x,y) = 0 \}$,
        \item $rg(\varphi) = \dim(E) - \dim(ker(\varphi))$.
    \end{enumerate}
}
\vocabulary{On dit que $\varphi$ est \textbf{non-dégénérée} si $ker(\varphi) = \{0\}$, c'est à dire si $rg(\varphi) = \dim(E)$.}

\theorem{Théorème}{}{false}{
    Soit $\varphi \in \mathcal{B}(E)$, supposons $\varphi$ symétrique.\\
    Soit $(e_1, \ldots, e_n)$ une base de $E$, et $A_{ij} = \varphi(e_i, e_j)$, alors on a :
    \begin{enumerate}
        \item $\varphi$ est non dégénérée si et seulement si $A$ est inversible,
        \item $ker(\varphi) = \{ x \in E \mid A X = 0 \}$,
        \item $rg(\varphi) = rg(A)$.
    \end{enumerate}
}

\noindent{\textbf{Démonstration :}\\
Posons $\varphi : E \times E \to \mathbb{R}$ telle que $\varphi : x \mapsto (y \mapsto \varphi(x,y))$.
Posons aussi $d : E \to E*$.\\
Comme $\varphi$ est bilinéaire, $d$ est linéaire.\\
Ainsi, $ker(\varphi) = ker(d)$ et $rg(\varphi) = rg(d)$.\\
...
\example{
    ...
    En effet, si $\varphi$ est un produit scalaire, alors $\varphi(x,y) = \varphi(y,x)$ pour tous $x,y \in E$, et $ker(\varphi) = \{0\}$ (car $\varphi(x,x) > 0$ pour tout $x \neq 0$).
}
...
\theorem{Proposition}{}{false}{
    Soit $\varphi \in \mathcal{B}(E)$, supposons $\varphi$ symétrique. Alors :
    \begin{enumerate}
        \item $\varphi$ est non-dégénérée si et seulement si $rg(\varphi) = \dim(E)$,
        \item Soit $B=(e_1,\ldots,e_n)$ une base de $E$. Alors $rg(Mat_B(\varphi)) = rg(\varphi)$.\\
        On a de plus que $\varphi$ est non-dégénérée si et seulement si $Mat_B(\varphi)$ est inversible.
    \end{enumerate}
}
...
\definition{
    Soit $E$ un $\mathbb{R}$-espace vectoriel. Soit $\varphi \in \mathcal{B}(E)$, supposons $\varphi$ symétrique. Alors :
    \begin{enumerate}
        ...
        Si de plus $\varphi$ est un produit scalaire, alors $H^\perp \cap H = \{0\}$.
        \item Un vecteur $x \in E$ est dit \textbf{isotrope} si $x \perp_\varphi x$. \textit{Autrement dit, $x$ est isotrope si $\varphi(x,x) = 0$.}
    \end{enumerate}
}
...
\theorem{Proposition}{}{false}{
    Soit $\varphi \in \mathcal{B}(E)$, supposons $\varphi$ symétrique. Soit $F$ un sous-espace vectoriel de $E$.\\
    Alors :
    \[
        dim(F^\perp_\varphi) = dim(E) - dim(F) + dim(F \cap ker(\varphi))
    \]
    Si $\varphi$ est non dégénérée, alors $dim(F^\perp_\varphi) = dim(E) - dim(F)$.
}

\noindent{\textbf{Démonstration :}\\
    On regarde l'application $D : \begin{matrix}    E\to E^*\\
                                                    x\mapsto (y\mapsto \varphi(x, y)) \end{matrix}$.\\
    $D$ est une application linéaire car $\varphi$ est bilinéaire. \\
    $x\in Ker(D)$, donc $D(x)=0$ si et seulement si $\forall y\in E, \varphi(x, y)=0$. Donc $Ker(D)=Ker(\varphi)$. \\
    Si $\varphi$ est non-dégénérée, alors $Ker(D)={0}$. Donc $D$ est un isomorphisme.\\
    Soit $L={\ell\in E^* : \ell_F|=0}$.\\
    Donc, $D(F^\bot)=L$. Donc $dim(F^{\bot_\varphi})=dim(L)=dim(E)-dim(F)$. (d'après la remarque en dessous)\\
}

\remark{ $D(x)\in L\Leftrightarrow \forall y\in F, D(x)(y)=0\Leftrightarrow \varphi(x, y)=0 \forall y\in F \Leftrightarrow x\in F^\bot$.}	

\theorem{Corollaire}{}{false}{
    Si $\varphi \in \mathcal{B}(E)$ est \textbf{symétrique et non dégénérée}, alors $\forall F \subseteq E, (F^\perp_\varphi)^\perp = F$.
}

\noindent{\textbf{Démonstration : }\\
    $dim((F^{\bot_\varphi})^{\bot_\varphi})=dim(E)-dim(F^{\bot_\varphi})=dim(E)-(dim(E)-dim(F))=dim(F)$.\\
    L'inclusion $F\subseteq (F^{\bot_\varphi})^{\bot_\varphi}$ est automatique car $x\in F$, alors $\forall y\in F^{\bot_\varphi}, \varphi(x, y)=0$, alors $x\in (F^{\bot_\varphi})^{\bot_\varphi}$.\\
}
...
\definition{
    Soit $E$ un $\mathbb{R}$-espace vectoriel.\\
    Une fonction $q : E \to \mathbb{R}$ est une \textbf{forme quadratique} sur $E$ s'il existe $\varphi \in \mathcal{B}(E)$ telle que $q(x) = \varphi(x,x)$ pour tout $x \in E$.
}
...
\theorem{Méthode du dédoublement}{}{false}{
    ...
    \begin{itemize}
        \item Les carrés sont remplacés par des produits mixtes : $x^2 \longrightarrow xx'$, $y^2 \longrightarrow yy'$, etc.
        \item Les termes croisés sont remplacés par la moyenne symétrique : $xy \longrightarrow \frac{1}{2}(xy' + x'y)$, $xz \longrightarrow \frac{1}{2}(xz' + x'z)$, etc.
    \end{itemize}
    Cette méthode assure que $\varphi$ est bilinéaire, symétrique et que $\varphi(u, u) = q(u)$.
}

\example{
    Soit $q(x, y, z) = x^2 + 2y^2 - 3z^2 + 4xy - 2xz$. 
...
\definition{
    ...
    \begin{itemize}
        ...
        \item $q$ est \textbf{non dégénérée} si $Ker(D_\varphi) = \{0\}$.
    \end{itemize}
}
...
\remark{
    ...
    \begin{itemize}
        \item $x$ est \textbf{isotrope} si $q(x) = 0$, ($Leftarrow$ $x \perp x$),
...
\example{
    Considérons $q(x,y,z) = x^2 - y^2 + 2z^2$ une forme quadratique sur $\mathbb{R}^3$.\\
...
\example{
    Soit $q(x,y,z) = x^2 + (y+z)^2$ une forme quadratique sur $E = \mathbb{R}^3$. \\
    ...
    \]
    La matrice de passage de la base canonique duale $B_c^*$ vers la base $B^*$ (contenant les coefficients des $\ell_i$ en colonnes) est :
    \[
        P_{B_c^* \to B^*} = \begin{pmatrix}
        1 & 0 & 0 \\
        0 & 1 & 0 \\
        0 & 1 & 1
        \end{pmatrix}.
    ...
    \[
        P_{B_c \to B} = ({}^t P_{B_c^* \to B^*})^{-1} = {}^t(P_{B_c^* \to B^*}^{-1}).
    \]
    Par pivot de Gauss, on calcule l'inverse :
    \[
        P_{B_c^* \to B^*}^{-1} = \begin{pmatrix} 
        1 & 0 & 0 \\ 
        0 & 1 & 0 \\ 
        0 & -1 & 1 
        \end{pmatrix} \implies {}^t(P_{B_c^* \to B^*}^{-1}) = \begin{pmatrix}
        1 & 0 & 0 \\    
        0 & 1 & -1 \\
        0 & 0 & 1
        \end{pmatrix}.
    \]
    On en déduit la matrice de passage $P_{B_c \to B}$. En lisant ses colonnes, la base antéduale est $B = (e_1, e_2, -e_2 + e_3)$. Cette base est, par construction, $q$-orthogonale.
}
...
\theorem{Identité de polarisation}{}{false}{
...
\noindent{\textbf{Démonstration du théorème : }\\
    ...
    Posons $F=Vect(e_1 ... e_p)$ et $G=Vect(e_{p+1} ... e_{p+r})$ ; 
    $F'=Vect(e_1' ... e_p')$ et $G'=Vect(e_{p'+1}' ... e_{p'+r'}')$.\\
    Montrons que $dim(F)=dim(F')$ et $dim(G)=dim(G')$.\\
    On sait que $dim(F)+dim(G)=p+r=p'+r'=dim(F')+dim(G')$. 
    Considérons $q_{|F\cap G'}$.\\
    Par le lemme, et $q(e_i)>0$ pour $1\le i\le p$, on obtient que $q$ est bien défini positif sur $F$.\\
    De même, par le lemme pour tout $p'+1\le i\le n$, $q(e_i)\le0$, donc $q$ est négative sur $G'$.\\
    Donc, $q_{|F\cap G'}$ est à la fois défini positif et négative, donc $F\cap G'=\{0\}$.
    De manière symétrique, on a $F'\cap G=\{0\}$.\\
    On a donc $dim(F)+dim(G')=dim(F+G')\le n$ et $dim(F')+dim(G)=dim(F'+G)\le n$\\
    $\Leftrightarrow p+n-p'\le n$ et $p'+n-p\le n$. Donc $p\le p'$ et $p'\le p$.\\
