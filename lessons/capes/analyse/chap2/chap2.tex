...
Il s'agit du cas où $a$ et $b$ sont des constantes réelles (avec $a \neq 0$). Quitte à diviser par $a$, on se ramène systématiquement à la forme canonique $x'(t) - \alpha x(t) = \beta(t)$.
...
\training{Résoudre sur $\mathbb{R}$ l'équation différentielle $x'(t) - 2x(t) = t + e^{2t}$ en combinant habilement les structures de solutions de la forme $P(t)e^{\omega t}$ et le principe de superposition.}
\noindent\carreaux{10}
...
\method{Résolution rigoureuse}{
    Pour résoudre de manière rigoureuse l'équation $x'(t) = g(t) \cdot h(x(t))$ sur un intervalle :
    \begin{enumerate}
        \item \textbf{Solutions stationnaires :} On identifie d'abord les racines de $h$. Si un réel $x_0 \in J$ vérifie $h(x_0) = 0$, la fonction constante $x(t) = x_0$ constitue une solution de l'équation.
        \item \textbf{Solutions non constantes :} Sur un intervalle d'étude où $h(x(t)) \neq 0$, on divise par $h(x(t))$ :
        \[
            \frac{x'(t)}{h(x(t))} = g(t)
        \]
        \item On procède à l'intégration temporelle des deux membres : $\int \frac{x'(t)}{h(x(t))} \, dt = \int g(t) \, dt$.
        \item Par application du changement de variable de classe $\mathcal{C}^1$ donné par $u = x(t)$ (impliquant $du = x'(t)dt$), on se ramène au calcul d'une primitive de fonctions à variables découplées :
        \[
...
\example{
    L'équation homogène linéaire à coefficients non constants $x'(t) = a(t)x(t)$ correspond au cas standard $g(t) = a(t)$ et $h(x) = x$. La fonction nulle $x(t) = 0$ est la solution stationnaire évidente. Sur tout intervalle ne contenant pas d'annulation, l'intégration donne $\ln|x| = A(t) + K$, menant aux solutions $x(t) = C e^{A(t)}$ avec $C \in \mathbb{R}$.
}

\training{Résoudre l'équation différentielle non linéaire à variables séparables suivante : $x'(t) = t \cdot x(t)^2$ sur un intervalle à définir avec précision, soumise à la condition initiale $x(0) = 1$.}
\noindent\carreaux{10}


\section{Équations linéaires du second ordre à coefficients constants}

On étudie les équations de la forme $(E) : a x''(t) + b x'(t) + c x(t) = d(t)$, où $a, b, c \in \mathbb{R}$ avec $a \neq 0$.
...
Soit $x$ une solution. Posons $g(t) = x(t)e^{-r_0 t}$, d'où $x(t) = g(t)e^{r_0 t}$. En dérivant successivement :
...
Une fonction de dérivée seconde nulle étant nécessairement affine, il existe $C_1, C_2 \in \mathbb{R}$ tels que $g(t) = C_1 t + C_2$. On en déduit la forme requise $x(t) = (C_1 t + C_2)e^{r_0 t}$. $\Box$}

\example{
    Résoudre sur $\mathbb{R}$ l'équation différentielle $x''(t) + x(t) = 0$. L'équation caractéristique est $r^2 + 1 = 0$, de racines complexes $r = \pm i$. La solution générale homogène s'écrit :
    \[
