...
\noindent{\textbf{Preuve :}\\
Soit $f : [a, b] \to \mathbb{R}$ une fonction \textbf{continue} avec $a < b$.\\
Alors $\exists m, M \in [a,b]$ (les minimums et maximums de $f$ sur $[a,b]$) tels que :
$$ m \le f(t) \le M, \quad \forall t \in [a, b]$$
En intégrant, on obtient :
$$ m(b-a) \le \int_{a}^{b} f(t) \, dt \le M(b-a)$$
Or $a < b$, donc $b-a > 0$, et on peut diviser par $b-a$ :
$$ m \le \frac{1}{b-a} \int_{a}^{b} f(t) \, dt \le M$$
On retrouve la définition de la valeur moyenne $\mu$, \textit{i.e.} : $m \le \mu \le M$.\\
Comme $f$ est continue, par le théorème des valeurs intermédiaires $\exists c \in [a,b] : f(c) = \mu$. Ainsi :
$$\int_{a}^{b} f(t) \, dt = (b-a) \mu = (b-a) f(c)$$
$\hfill \Box$
}
...
\noindent Il est mis à disposition selon les termes de la Licence Creative Commons \ccbyncsa\ \textbf{Attribution - Pas d'Utilisation Commerciale - Partage dans les Mêmes Conditions 4.0 International}. 
