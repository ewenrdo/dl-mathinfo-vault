...
\theorem{Théorème}{Moindre carré}{false}{
    La moyenne $\bar{x}$ est l'unique réel $\theta$ qui minimise la fonction $g : \theta \mapsto \sum_{i=1}^{k} n_i (x_i - \theta)^2$.
}
...
\theorem{Théorème}{Optimisation en valeur absolue}{false}{
    La médiane $M_e$ (ou tout réel médian au sens de la définition si $N$ est pair) est un réel $\theta$ qui minimise la fonction $h : \theta \mapsto \sum_{i=1}^{k} n_i |x_i - \theta|$.
}
