...
\theorem{Théorème}{Théorème fondamental de l'arithmétique}{false}{
...
\noindent{\textbf{Preuve :} Il faut démontrer l'existence puis l'unicité.
\begin{itemize}
    \item \textbf{Existence :} On procède par récurrence forte sur $n \ge 2$.\\
    \begin{enumerate}
        \item \textbf{Initialisation :} Pour $n = 2$, on a $2 = 2^1$ avec $2 \in \mathbb{P}$.
        \item \textbf{Hérédité :} Supposons que tout entier $k$ tel que $2 \le k \le n$ s'écrit comme un produit de nombres premiers.\\
        On veut montrer que $n+1$ s'écrit comme un produit de nombres premiers.\\
        \begin{itemize}
            \item Si $n+1$ est premier, alors $n+1 = (n+1)^1$ est un produit de nombres premiers.
            \item Sinon, $n+1$ n'est pas premier. Donc il existe $a, b \in \mathbb{N}^*$ tels que $2 \le a, b < n+1$ et $n+1 = a \cdot b$.\\
            Par hypothèse de récurrence, $a$ et $b$ s'écrivent comme des produits de nombres premiers. Ainsi, $n+1 = a \cdot b$ s'écrit également comme un produit de nombres premiers.
        \end{itemize}
        \item \textbf{Conclusion :} Par le principe de récurrence \textit{(forte)}, tout entier $n \ge 2$ s'écrit comme un produit de nombres premiers.
    \end{enumerate}
    \item \textbf{Unicité :} Supposons que $n \ge 2$ s'écrive de deux manières différentes, mettons $n = p_1 \cdots p_r = q_1 \cdots q_s$ avec $p_i, q_j \in \mathbb{P}$ \textit{(cette écriture est équivalente à l'écriture avec les exposants $\alpha_i$)}.\\
    Il vient $\exists j \in [1, s] \cap \mathbb{N}, p_1 \mid q_j$. Supposons sans perte de généralité que $j = 1$. Alors $p_1 \mid q_1$ et comme $q_1$ est premier, il vient $p_1 = q_1$.\\
    On peut donc simplifier par $p_1 = q_1$ et on obtient $p_2 \cdots p_r = q_2 \cdots q_s$.\\
    En répétant ce raisonnement, on obtient que $r = s$ et que les $p_i$ sont exactement les $q_j$ à l'ordre près. Ainsi, l'écriture en produit de nombres premiers est unique. $\hfill \square$
\end{itemize}
}
...
\remark{
    Le test deterministe jusqu'à $\sqrt{n}$ devient inutilisable pour de très grands entiers (comme ceux utilisés en cryptographie, ayant plusieurs centaines de chiffres). En pratique, on utilise des \textbf{tests de primalité probabilistes} (comme le test de Fermat ou de Miller-Rabin) fondés sur des propriétés congruentielles.
}
...
\noindent{\textbf{Preuve :} Soient $p$ un nombre premier et $a \in \mathbb{Z}$ tel que $p \nmid a$.\\
L'ensemble des restes non nuls modulos $p$ est $E = \{1, 2, \ldots, p-1\}$. Considérons également $aE = \{a, 2a, \ldots, (p-1)a\}$.\\\\
Supposons que $\exists 1 \leq j < k \leq p-1 \cap \mathbb{N}$ tels que $ka \equiv ja \; [p] \Leftrightarrow k - j \equiv 0 \; [p]$ \textit{(car $\pgcd(a,p) = 1$, lemme de Gauss)}. Comme $k - j < p$, $p \nmid (k -j)$, absurde !
Ainsi, les éléments de $aE$ sont tous distincts modulo $p$. A fortiori, ils sont tous non nuls modulo $p$.\\
...
\textit{(ce dernier point étant justifié par le lemme de Gauss car $\pgcd((p-1)!, p) = 1$).}
Le cas particulier $a^p \equiv a \; [p]$ s'obtient en multipliant les deux membres de l'équivalence précédente par $a$. $\hfill \square$
}
...
\theorem{Lemme}{Binôme modulo $p$}{true}{
    Soient $x,y \in \mathbb{Z}$ et soit $p \in \mathbb{P}$.\\
    Alors :
    \[
...
\noindent{\textbf{Preuve :}\\
    Soient $x,y \in \mathbb{Z}$ et $p \in \mathbb{P}$.\\
    Par le binôme de Newton, on a :
    \[
        (x + y)^p = \sum_{k=0}^{p} \binom{p}{k} x^k y^{p-k}
    \]
    Or, pour $0 < k < p$, le coefficient binomial $\binom{p}{k}$ est divisible par $p$ (car il contient $p$ au numérateur et aucun facteur du dénominateur ne le divise). Ainsi, tous les termes du développement sauf $x^p$ et $y^p$ sont congrus à $0$ modulo $p$. Donc :
    \[
        (x + y)^p \equiv x^p + y^p \; [p]
    \]
    $\hfill \square$
...
\remark{Pour la culture, pour calculer $a^b$ modulo $n$ lorsque $b$ est très grand, on utilise le théorème d'Euler (ou de Fermat) pour réduire d'abord l'exposant $b$, puis l'algorithme d'\textbf{exponentiation rapide} (qui réduit le nombre de multiplications via la décomposition binaire de $b$).}
...
\theorem{Théorème}{Théorème d'Euler}{true}{
    Soient $n \in \mathbb{N}^*$ et $a \in \mathbb{Z}$ tels que $\pgcd(a, n) = 1$.\\
    Alors on a :
    \[
...
\noindent{\textbf{Preuve :} Soient $n \in \mathbb{N}^*$ et $a \in \mathbb{Z}$ tels que $\pgcd(a, n) = 1$.\\
On considère l'ensemble $S = \{ k \in \{1, 2, \dots, n\} \mid \pgcd(k, n) = 1\}$. Il vient $Card(S) = \varphi(n)$ (par défintiion de l'indicatrice d'Euler).\\
On considère également l'ensemble $aS = \{ ak \mid k \in S\}$. Comme $\pgcd(a, n) = 1$, le lemme de Gauss assure que les éléments de $aS$ sont tous distincts modulo $n$. De plus, comme $\pgcd(k, n) = 1$ pour tout $k \in S$, on a que $\pgcd(ak, n) = 1$ pour tout $k \in S$. Ainsi, les éléments de $aS$ sont tous distincts et premiers avec $n$ modulo $n$. A fortiori, ils sont tous dans $S$.\\
Ainsi, les éléments de $aS$ et $S$ sont les mêmes modulo $n$. En particulier, on a :
\[
    \prod_{k \in S} ak \equiv \prod_{k \in S} k \; [n]
\]
\[
    \Leftrightarrow a^{\varphi(n)} \prod_{k \in S} k \equiv \prod_{k \in S} k \; [n]
\]
\[
    \Leftrightarrow a^{\varphi(n)} \equiv 1 \; [n]
\]
\hfill$\square$
}

\theorem{Proposition}{Propriété de la somme}{true}{
...
\remark{
    Le petit théorème de Fermat est un cas particulier du théorème d'Euler : lorsque $n = p$ est premier, $\varphi(p) = p - 1$, ce qui donne bien $a^{p-1} \equiv 1 \; [p]$.
}

\training{
    Déterminer les deux derniers chiffres de $7^{402}$ en utilisant le théorème d'Euler modulo $100$.
}
\noindent \carreaux{7}
