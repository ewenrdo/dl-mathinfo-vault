...
\remark{Ces exceptions sont de type \texttt{exn}, qui est un type prédéfini dans OCaml. Elles peuvent être levées automatiquement par le système ou manuellement par le programmeur (via \texttt{raise} ou \texttt{failwith}).}
...
\example{Exemple d'une fonction qui lève une exception prédéfinie :}
\begin{lstlisting}[language=Caml]
let rec mem x l =
  match l with
  | [] -> raise Not_found
  | h :: r -> if h = x then true else mem x r;;
\end{lstlisting}
