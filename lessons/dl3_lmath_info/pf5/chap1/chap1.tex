...
\disclaimer{En examen, elle demandera de trouver les types des expressions données. Attention à ne pas passer ça (et la forme des types de fonctions) sous silence.}
...
\method{Déclaration d'une variable ou d'une fonction globale}{
    On utilise le mot-clé \texttt{let} pour lier un nom à une valeur ou à une fonction de manière permanente au niveau global du fichier.
    
    Syntaxe d'une variable globale : \texttt{let nom = expression}
    
    Syntaxe d'une fonction globale : \texttt{let nom\_fonction arg1 arg2 = corps}
}
...
\subsubsection{Liaisons locales : la structure \texttt{let ... in ...}}

\definition{Une liaison locale permet de définir une variable ou une fonction intermédiaire dont la portée (visibilité) est restreinte exclusivement à une expression spécifique. On utilise pour cela le mot-clé \texttt{let ... in ...}.}

\remark{Contrairement à un \texttt{let} global, un \texttt{let ... in} est lui-même une \textbf{expression} et produit une valeur de retour (celle de la partie après le \texttt{in}). Elle permet d'éviter de polluer l'espace de nom global du programme et s'avère indispensable pour décomposer des calculs ou forcer un ordre d'évaluation lors d'effets de bord.}

\method{Structure du \texttt{let ... in}}{
    La syntaxe générale est la suivante :
    \[
    \texttt{let } nom = expression\_locale \texttt{ in } expression\_principale
    \]
    Le \texttt{nom} n'est accessible et connu que dans l'\textit{expression\_principale}. Dès que celle-ci est évaluée, la variable locale est oubliée et détruite de la mémoire.
}

\example{Exemple d'utilisation de liaisons locales pour calculer la racine d'un polynôme ou décomposer une formule géométrique :}
\begin{lstlisting}[language=Caml]
(* Calcul du perimetre d'un rectangle avec des variables locales *)
let perimetre la lo =
  let double_largeur = 2 * la in
  let double_longueur = 2 * lo in
  double_largeur + double_longueur

(* double_largeur n'existe plus ici, l'appeler provoquerait une erreur *)
\end{lstlisting}
...
\section{Digression : utilisation du \texttt{";"} et du \texttt{";;"}}

Comme on l'a dit, chaque expression en OCaml renvoie une valeur. Il convient de ne pas confondre le rôle du point-virgule simple \texttt{;} et du double point-virgule \texttt{;;}.

\begin{itemize}
    \item \textbf{Le point-virgule simple (\texttt{;}) :} C'est un \textbf{séquenceur d'instructions}. Il s'utilise uniquement à l'intérieur d'un bloc de code pour enchaîner des expressions (généralement de type \texttt{unit}) possédant des effets de bord. L'expression de gauche est évaluée, son résultat est jeté, puis l'expression de droite est exécutée. La dernière expression d'un bloc ne prend jamais de point-virgule simple.
    \item \textbf{Le double point-virgule (\texttt{;;}) :} C'est un \textbf{délimiteur de phrase globale}. Il sert principalement dans la console interactive (le REPL, c'est-à-dire le mode interactif du compilateur) pour signifier au compilateur que la saisie est terminée et qu'il peut exécuter le bloc. Dans un fichier source (\texttt{.ml}), il est facultatif si les déclarations globales s'enchaînent par des mots-clés clairs (\texttt{let}), mais il reste utile pour exécuter directement des instructions libres au niveau global.
\end{itemize}

\example{
    Pour afficher deux messages consécutifs et clore la fonction dans le REPL, on combine les deux syntaxes :
}
\begin{lstlisting}[language=Caml]
let saluer () =
  print_string "Hello, "; (* Le ";" separe deux expressions unit *)
  print_string "world !"  (* Pas de ";" sur la derniere expression *);; 
  (* Le ";;" indique a l'interpreteur que la definition est finie *)
\end{lstlisting}
