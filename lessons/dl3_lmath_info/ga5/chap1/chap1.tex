...
\example{Quelques notations typiques pour les lois de composition interne : \(+\), \(\times\), \(\cdot\), \(\circ\), etc.}
...
\theorem{Propriété}{Unicité du symétrique}{false}{
    Soit \((E, *)\) un ensemble muni d'une loi de composition interne associative. Supposons que $*$ admet un élément neutre $e$. Si $x \in E$ admet un symétrique pour $*$, alors ce symétrique est unique.
}
...
\theorem{Propriété}{Bijection de multiplication}{false}{
...
\noindent{\textbf{Preuve à gauche :} Montrons que $\ell_x : G \to G$ définie par $\ell_x(y) = x * y$ est injective et surjective.\\
\begin{itemize}
    \item \textbf{Injectivité :} Soient $y,z \in G$ tels que $\ell_x(y) = \ell_x(z)$, c'est-à-dire $x * y = x * z$.\\
    $$ y = x^{-1} * (x * y) = x^{-1} * (x * z) = (x^{-1} * x) * z = e * z = z $$
    Donc $\ell_x$ est injective.

    \item \textbf{Surjectivité :} Soit $z \in G$. On veut trouver $y \in G$ tel que $\ell_x(y) = z$, c'est-à-dire $x * y = z$.\\
    En multipliant à droite par l'inverse de $x$, on obtient $y = z * x^{-1}$. Donc $\ell_x$ est surjective.
\end{itemize}
...
\begin{enumerate}
    ...
    \item L'application $r_x : E \to E$ définie par $r_x(y) = y * x$ est bijective (multiplication à droite).
\end{enumerate}
}
\noindent\carreaux{10}

\method{Montrer qu'une application est injective}{
    Pour montrer qu'une application \(f : A \to B\) est injective, il suffit de montrer que si \(f(x_1) = f(x_2)\), alors \(x_1 = x_2\).\\
    Autrement dit, on suppose que deux éléments de l'ensemble de départ ont la même image et on montre qu'ils sont égaux.
}

\method{Montrer qu'une application est surjective}{
    Pour montrer qu'une application \(f : A \to B\) est surjective, il suffit de montrer que pour tout élément \(y \in B\), il existe un élément \(x \in A\) tel que \(f(x) = y\).\\
    Autrement dit, on prend un élément quelconque de l'ensemble d'arrivée et on montre qu'il existe un élément dans l'ensemble de départ qui lui correspond.
}

\remark{Jusqu'ici, on a déjà démontré beaucoup de choses qui ne sont pas uniquement vraies pour les groupes, mais pour tout ensemble muni d'une loi de composition interne associative avec un élément neutre.}
...
\begin{itemize}
    ...
    \item Il existe un élément neutre \(e_G \in G\).
...
\example{
    \begin{itemize}
        ...
        \item $Bij(X) = S(X) = \left( \{ \text{bijections } X \to X \mid X \text{ est un ensemble} \}, \circ \right)$ : l'ensemble des bijections d'un ensemble \(X\) dans lui-même avec la composition (si $X = \{1, 2, \ldots, n\}$, on note $S(X) = S_n$)\footnote{cf. \href{https://www.ewenrdo.fr/ressources?slug=algebre-3-chapitre-2-groupe-symetrique}{Chapitre 2 : Groupe symétrique} d'AL3, en particulier pour les cycles et les permutations qui ne sont pas rappelées ici.}.
        \item $GL_n(\mathbb{K}) = \left( \{ A \in M_n(\mathbb{K}) \mid \det(A) \neq 0 \}, \times \right)$ : l'ensemble des matrices carrées inversibles d'ordre \(n\) avec la multiplication matricielle.
    \end{itemize}
}
\cexample{
    \begin{itemize}
        \item \((\mathbb{N}, +)\) : l'ensemble des entiers naturels avec l'addition (pas d'élément neutre dans \(\mathbb{N}\)).
        \item \((\mathbb{Z}, \times)\) : l'ensemble des entiers avec la multiplication (pas de symétrique pour les entiers autres que \(1\) et \(-1\)).
        \item \(\mathbb{R}\) avec la multiplication (pas de symétrique pour \(0\)).
    \end{itemize}
}
...
\remark{
    ...
    \begin{itemize}
        ...
        Par récurrence, on note aussi : $\begin{cases}
            x^{n+1} = x^n \cdot x & \text{si } n \geq 0 \\
            x^0 = 1 & \\
            x^n = (x^{-n})^{-1} & \text{si } n < 0
        \end{cases}$
        \item Composée de fonctions : on note le groupe \((G, \circ)\), l'élément neutre est noté \(id_G\) et le symétrique \textit{(appelé l'inverse)} de \(f\) est noté \(f^{-1}\).\\
    \end{itemize}
}

\newpage

\training{Soient $n,m \in \mathbb{Z}$. Montrer par récurrence que $x^n \cdot x^m = x^{n+m}$ et $x^{nm} = (x^n)^m$. On pourra distinguer les cas $\mathbb{N}$ et $\mathbb{Z}$ lorsque nécessaire.\\
\textbf{Indication :} Pour les négatifs, considérer les cas \(n < 0, m \geq 0\), \(n \geq 0, m < 0\) et \(n < 0, m < 0\) séparément, et poser $n' = -n$ et $m' = -m$ pour les cas négatifs.
}
\noindent\carreaux{6}

\ndlr{À cet endroit, il y a eu un rappel sur les permutations et la décomposition de permutations en cycles à supports disjoints, ainsi que sur $S_n$ et sa non-commutativité (et son cardinal). Le ré-ajouter ?}

\definition{
    Soit $(G, *)$ un groupe où $G$ est de cardinal \textbf{fini}.\\
    On appelle \textbf{ordre} de $G$ le cardinal de $G$ et on le note $|G|$. On dit que $G$ est un groupe fini.
}

\example{$|S_n| = n!$ pour tout $n \in \mathbb{N}^*$.}

\remark{Il a été vu en cours une introduction à $\mathbb{Z}/n\mathbb{Z}$, se référer au chapitre 3 pour plus de détails\footnote{\href{https://www.ewenrdo.fr/ressources?slug=ga5-chapitre-3-exemples-de-groupes}{Chapitre 3 : Exemples de groupes importants}.}.}
...
\vocabulary{Si $\varphi$ est un morphisme de groupes et est bijective, alors $\varphi$ est un \textbf{isomorphisme} (si de plus $\varphi$ est un endomorphisme, alors $\varphi$ est un \textbf{automorphisme}).}
\theorem{Proposition}{Propriétés d'un morphisme de groupes}{false}{
    ...
    \begin{itemize}
        ...
        \item $\forall k \in \mathbb{Z}, \varphi(g^k) = (\varphi(g))^k$.
    \end{itemize}
}
...
\noindent{\textbf{Preuve :} Soient $(G, \cdot), (G', *), (G'', \times)$ deux groupes.\\
    $(g\circ f)(x\cdot y)=g(f(x\cdot y))=g(f(x)*f(y))=g(f(x))\times g(f(y))=(g\circ f)(x)\times (g\circ f)(y)$
}

\theorem{Proposition}{Isomorphisme et morphisme inverse}{false}{
    Soit $\varphi$ un morphisme de groupes.\\ 
...
\theorem{Proposition}{Groupe des automorphismes}{false}{
    Soit $G$ un groupe. Alors $(Aut(G), \circ)$ est un groupe, où $Aut(G)$ est l'ensemble des automorphismes de $G$ et $\circ$ est la composition de fonctions.
}
\noindent{\textbf{Preuve :} La composée de deux automorphismes est un automorphisme, l'élément neutre est l'identité $id_G$, et l'inverse d'un automorphisme est aussi un automorphisme. $\Box$}

\ndlr{Fin du cours du 07/09/2026.}
...
\theorem{Proposition}{Injectivité}{false}{
    Soit $\varphi$ un morphisme de groupes. $\varphi$ injective $\Leftrightarrow Ker(\varphi) = \{ e \}$.
}

\noindent{\textbf{Démonstration :}
    \begin{itemize}
        \item "$\Rightarrow$" : On a $f(e_G)=e_H$, donc par injectivité de $f$, $Ker(f)=f^{-1}({e_H})={e_G}$ ;
        \item "$\Leftarrow$" : Soient $x, y\in G$ tels que $f(x)=f(y)$.\\
            Alors, $f(x) *_H f(y)^{-1}=e_H \Leftrightarrow f(x) *_H f(y^{-1})=e_H \Leftrightarrow f(x *_G y^{-1})=e_H$\\
            $x *_G y^{-1}\in Ker(f)={e_G}$, donc $x *_G y^{-1}=e_G$, donc $x=y$.\\
            Donc, $f$ est injectif.
    \end{itemize}
}
...
\theorem{Corollaire}{Image et image réciproque d'un sous-groupe par un morphisme}{true}{
    Soit $H \subset G$ un sous-groupe.\\
    Alors $\varphi(H)$ est un sous-groupe de $G'$. De même, si $H' \subset G'$ est un sous-groupe, alors $\varphi^{-1}(H')$ est un sous-groupe de $G$.
}
\noindent{\textbf{Preuve :} \begin{enumerate}
    \item Soit $K$ un sous-groupe de $G$.
    \begin{itemize}
        \item \textbf{Élément neutre (ensemble non vide) :} On a $f(e_G) = e_H$, et $e_G \in K$ car $K < G$. Donc $e_H \in f(K)$.
        \item \textbf{Stabilité par la loi et l'inverse :} Soient $x, y \in f(K)$. Soient $a, b \in K$ tels que $x = f(a)$ et $y = f(b)$.
        \[
        \text{Alors, } x^{-1} *_H y = f(a)^{-1} *_H f(b) = f(a^{-1}) *_H f(b) = f(a^{-1} *_G b) \in f(K).
        \]
    \end{itemize}
    On conclut que $f(K) < H$.

    \item Soit $K < H$.
    \begin{itemize}
        \item \textbf{Élément neutre (ensemble non vide) :} On a $e_H \in K$ car $K < H$ et $f(e_G) = e_H$. Donc $e_G \in f^{-1}(K)$.
        \item \textbf{Stabilité par la loi et l'inverse :} Soient $x, y \in f^{-1}(K)$. Alors $f(x^{-1} *_G y) = f(x^{-1}) *_H f(y) = f(x)^{-1} *_H f(y) \in K$ (car $f(x), f(y) \in K$ et $K < H$). Donc $x^{-1} *_G y \in f^{-1}(K)$.
    \end{itemize}
    Ainsi, $f^{-1}(K) < G$.
\end{enumerate}}
...
\theorem{Théorème de Lagrange}{Ordre d'un sous-groupe}{true}{
    Soit $(G, \cdot)$ un groupe fini et $(H, \cdot)$ un sous-groupe de $G$. Alors l'ordre de $H$ divise l'ordre de $G$.
}

\example{
    Soient \(G = S_3\) le groupe des permutations de l'ensemble \(\{1, 2, 3\}\) et \(H = \{ e, (1\ 2) \}\) un sous-groupe de \(G\). On a \(|G| = 6\) et \(|H| = 2\). Selon le théorème de Lagrange, l'ordre de \(H\) divise l'ordre de \(G\), ce qui est bien le cas puisque \(2\) divise \(6\).
}


\theorem{Proposition}{Image et image réciproque par un morphisme}{false}{
    Soit \functionSets{\varphi}{G \rightarrow G'} un morphisme de groupes. Alors :
    \begin{itemize}
        \item Si $H$ est un sous-groupe de $G$, alors $\varphi(H)$ est un sous-groupe de $G'$.
        \item Si $H'$ est un sous-groupe de $G'$, alors $\varphi^{-1}(H')$ est un sous-groupe de $G$.
    \end{itemize}
}

\noindent{\textbf{Démonstration : }
    \begin{enumerate}
        \item On utilise la caractérisation. Soit $K$ un sous-groupe de $G$. 
        \begin{itemize}
            \item On a $f(e_G)=e_{G'}$ et $e_{G'}\in K$ car $K\le G$. Donc $e_{G'}\in G$
            \item Soit $x, y\in f(K)$. Soient $a, b\in K$ tels que $x=f(a)$ et $y=f(b)$.\\
                Alors, $x^{-1} *_{G'} y = f(a)^{-1} *{G'} f(b) = f(a^{-1}) *_{G'} f(b) = f(a^{-1} *_G b)\in f(K)$. On conclut que $f(K)\le G'$.
        \end{itemize}
        \item Soit $K\le H$.
        \begin{itemize}
            \item On a $e_{G'}\in K$ car $K\le G'$ et $f(e_G)=f(e_{G'})$. Donc $e_G\in f^{-1}(K)$.
            \item Soient $x, y\in f^{-1}(K)$. Alors $f(x^{-1} *_G y) = f(x^{-1}) *_{G'} f(y) = f(x)^{-1} *_{G'} f(y) \in K$ (car $f(x), f(y)\in K$ et $K\le G'$). \\
            Donc $x^{-1} *_G y\in f^{-1}(K)$. Ainsi, $f^{-1}(K)\le G$.
        \end{itemize}
    \end{enumerate}
}
...
\theorem{Proposition}{Sous-groupe engendré par un élément}{false}{
...
\begin{itemize}
    \item Soit $n \in \mathbb{N}$ et vérifions que $n\mathbb{Z} < \mathbb{Z}$.
    \begin{align*}
        (Q_1) : & \quad 0 = n \cdot 0 \in n\mathbb{Z}. \\
        (Q_2) : & \quad \text{Si } x, y \in n\mathbb{Z}, \text{ on peut écrire } x = nk \text{ et } y = n\ell \text{ avec } k, \ell \in \mathbb{Z}, \\
                & \quad \text{et alors } -x + y = -nk + n\ell = n(\ell - k) \in n\mathbb{Z}.
    \end{align*}
    Donc, $n\mathbb{Z} < \mathbb{Z}$.

    \item Soit maintenant $G$ un sous-groupe de $(\mathbb{Z}, +)$.
    \begin{itemize}
        \item $1^\text{e}$ cas : $G = \{0\} = 0 \times \mathbb{Z}$ ;
        \item $2^\text{e}$ cas : $G \neq \{0\}$. Alors $G$ contient un élément non nul $x_0$. Quitte à remplacer $x_0$ par $-x_0$, on peut supposer que $x_0 > 0$.
        On peut alors choisir $y_0$ le plus petit élément de $G \cap \mathbb{N}^*$. Montrons que $G = y_0\mathbb{Z}$.
        
        Comme $y_0 \in G$, pour $k \in \mathbb{N}$, $k y_0 = \underbrace{y_0 + \dots + y_0}_{\times k} \in G$, d'où :
        
        $$(-k)y_0 = -(ky_0) \in G,\text{ donc } G \supseteq y_0\mathbb{Z}$$.
        
        Soit maintenant $x \in G$, on écrit la division euclidienne de $x$ par $y_0$ :
        \[
        x = q y_0 + r \quad \text{avec } 0 \le r \le y_0 - 1.
        \]
        Alors, $r = \underset{\in G}{\underbrace{x}} - q \underset{\in G}{\underbrace{y_0}} \in G$ car $G$ est un sous-groupe de $\mathbb{Z}$.
        
        On a donc trouvé $r \in G \cap \{0, \dots, y_0 - 1\}$.
        
        Par minimalité de $y_0$, $r = 0$ et donc $x = q y_0 \in y_0\mathbb{Z}$.
        
        Ainsi $G = y_0\mathbb{Z}$.
    \end{itemize}
\end{itemize}
}
...
\noindent{\textbf{Preuve:}\\
Soit $n = ord(y)$ On peut écrire la division euclidienne de $m$ par $n$ : $m = qn + r$, avec $0 \leq r < n$.\\
Il vient : $$y^m = y^{qn + r} = (y^n)^q * y^r = e^q * y^r = y^r$$
Or $n$ est le plus petit entier positif tel que $y^n = e$, donc $r = 0$. Ainsi, $m = qn$, ce qui montre que $n$ divise $m$. $\Box$
}
...
Merci à Laurent Cai à son aide dans la rédaction des démonstrations, ainsi qu'à Alex Theophilos pour ses retours sur des erreurs du cours.\\\\
