...
\noindent{\textbf{Preuve :}
    \begin{itemize}
        \item Si $\langle u, v\rangle=0$, alors on a $|\langle u, v| \rangle\le \|u\|\cdot\|v\|$ de façon triviale.
        \item Supposons que $\langle u, v\rangle=0$ et donc en particulier $u$ et $v$ sont non nuls.\\
        Pour tout $z\in \mathbb{C}$, on a $0\le\|u+zv\|^2=\|u\|^2+2\Re(z\langle u, v\rangle)+|z|^2\|v\|^2$.\\
        Pour tout $t\in \mathbb{R}$, posons $z_t=t\cdot\frac{|\langle u, v\rangle|}{\langle u, v\rangle}$. Ainsi, on a $0\le \|u+z_tv\|^2\le \|u\|^2+2t|\langle u, v\rangle|+t^2\|v\|^2$.\\
        Le membre de droite est un tronôme positif en $t$. Comme $\|v\|\ne 0$, alors le discriminant est négatif ou nul, où encore $|\langle u, v\rangle|-\|u\|^2\|v\|^2\le 0$ qui n'est rien d'autre que l'inégalité de Cauchy-Schwarz.
        \item  De plus, si $|\langle u, v\rangle|^2-\|u\|^2\|v\|^2=0$, alors il existe $z_t$ tel que $u+z_t v=0$. \\
        Réciproquement, si $u=\lambda v$, alors $|\langle u, v\rangle|=\|u\|\cdot\|v\|$.
    \end{itemize}
}
...
\theorem{Proposition}{Norme induite par une application linéaire injective}{false}{
    Soient $(E, \| \cdot \|_E)$ un espace vectoriel normé, $F$ un espace vectoriel et $f : F \to E$ une application linéaire.
    L'application $N = \| \cdot \|_E \circ f$ est une norme sur $F$ si et seulement si $f$ est injective.
}

\noindent{\textbf{Preuve :}

\begin{itemize}
    \item[$\Rightarrow/$] Supposons que $N = \|\cdot\|_F \circ f$ soit une norme sur $E$.
    Soit $x \in \ker(f)$, c'est-à-dire $f(x) = 0_F$.
    Alors $N(x) = \|f(x)\|_F = \|0_F\|_F = 0$.
    Puisque $N$ est une norme sur $E$, l'axiome de séparation implique que $x = 0_E$.
    Ainsi, $\ker(f) = \{0_E\}$, ce qui prouve que $f$ est injective.

    \item[$\Leftarrow/$] Supposons que $f$ soit injective. Vérifions que $N$ satisfait les trois axiomes d'une norme sur $E$ :
    \begin{enumerate}
        \item \textbf{Séparation :} 
        Soit $x \in E$.
        \begin{align*}
            N(x) = 0 &\iff \|f(x)\|_F = 0 \\
                     &\iff f(x) = 0_F \quad \text{(car } \|\cdot\|_F \text{ est une norme sur } F\text{)} \\
                     &\iff x \in \ker(f) \\
                     &\iff x = 0_E \quad \text{(car } f \text{ est injective)}.
        \end{align*}

        \item \textbf{Homogénéité :} 
        Soient $x \in E$ et $\lambda \in \mathbb{K}$.
        \begin{align*}
            N(\lambda x) &= \|f(\lambda x)\|_F \\
                         &= \|\lambda f(x)\|_F \quad \text{(par linéarité de } f\text{)} \\
                         &= |\lambda| \cdot \|f(x)\|_F \quad \text{(par homogénéité de } \|\cdot\|_F\text{)} \\
                         &= |\lambda| N(x).
        \end{align*}
        ...
        \begin{align*}
            N(x + y) &= \|f(x + y)\|_F \\
                     &= \|f(x) + f(y)\|_F \quad \text{(par linéarité de } f\text{)} \\
                     &\le \|f(x)\|_F + \|f(y)\|_F \quad \text{(par subadditivité de } \|\cdot\|_F\text{)} \\
                     &= N(x) + N(y).
        \end{align*}
    \end{enumerate}
    Ainsi, $N$ définit bien une norme sur $E$. \hfill $\square$
\end{itemize}}
...
\vocabulary{    Soit $A \subset E$. Une \textbf{suite} d'éléments de $A$ est une application $k \in \mathbb{N} \mapsto x_k \in A$, souvent notée $(x_k)_{k \in \mathbb{N}}$.}

\definition{
    On dit d'une suite $(x_k)_{k \in \mathbb{N}}$ qu'elle est \textbf{bornée} si :
    $$\exists r > 0 : \forall k \in \mathbb{N}, x_k \in B(0,r)$$
    ou en terme de normes : $\exists r > 0 : \forall k \in \mathbb{N}, \|x_k\| < r$.
}
...
\attention{La convergence dépend de la norme choisie. Ainsi, une suite peut converger pour une norme et ne pas converger pour une autre norme (sauf en dimension finie, vous l'aurez compris).}
...
\theorem{Proposition}{Convergence et équivalence des normes}{false}{
    ...
    En particulier, si $dim(E) < +\infty$, la convergence d'une suite ne dépend pas du choix de la norme.
}
...
\noindent{\textbf{Preuve :} \begin{itemize}
    \item[$\Rightarrow/$] Soit $\{x_k\}_{k \in \mathbb{N}}$ une suite convergente vers la limite $x \in E$. Alors
...
ce qui implique que $x_{\varphi(k)} \to x$. \hfill $\square$
    \item[$\Leftarrow/$] cf. Laurent pour la preuve.
\end{itemize}}

\method{Montrer qu'une suite n'est pas convergente}{
    Il suffit de trouver deux sous-suites convergeant vers des limites différentes.
}
