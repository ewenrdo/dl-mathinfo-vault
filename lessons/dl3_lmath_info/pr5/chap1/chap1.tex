...
\section{Tribus et espaces mesurables}
...
\subsection{Propriétés des tribus}
...
\section{Tribus engendrées et tribus boréliennes}

\subsection{Notion de tribu engendrée}
...
\subsection{Exemples et étude de la tribu de Borel sur $\mathbb{R}^d$}
...
\example{
    \begin{enumerate}
        ...
        \item Si $\Omega$ est muni de $\mathcal{F}$ engendrée par une partition finie ou dénombrable de $\Omega$, caractériser $\mu$ sur $(\Omega, \mathcal{F})$ revient à se donner $m_k = \mu(A_k)$ pour tout $k \in \mathbb{N}$ : On aura $\mu(\bigcup_{k \in I} A_k) = \sum_{k \in I} m_k$ pour tout $I \subseteq \mathbb{N}$.\\
...
\begin{tikzpicture}[scale=0.85, >=stealth]
  ...
  % Point x et sa boule B(x, eps_x) strictly incluse dans O
...
\section{Mesures positives et probabilités}

\subsection{Définitions et cadres d'étude}
...
\subsection{Propriétés fondamentales des mesures}
...
\section{La mesure de Lebesgue sur $\mathbb{R}^d$}
